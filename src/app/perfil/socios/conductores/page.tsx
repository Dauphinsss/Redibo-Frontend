...
export default function MisConductores() {
  ...
  const [selectedPartner, setSelectedPartner] = useState<Conductor | null>(null);
  ...
  useEffect(() => {
    ...
    if (!roles.includes("RENTER")) {
      setError("Acceso denegado. Solo los usuarios con rol RENTER pueden ver esta sección.");
      setLoading(false);
      return;
    }
    ...
    const fetchConductores = async () => {
      ...
      try {
        ...
        const sortedConductores = res.data.partners ? [...res.data.partners].sort((a: Conductor, b: Conductor) => a.nombre.localeCompare(b.nombre)) : [];
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      ...
      <p className="mb-6 text-gray-600">Gestiona y revisa la información de tus conductores asociados</p>
      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="animate-spin w-8 h-8 text-gray-400" /></div>
      ) : error ? (
        <div className="text-center text-red-500 py-8">{error}</div>
      ) : conductores.length === 0 ? (
        <div className="text-center text-gray-500 py-8">Aún no tienes conductores asociados. <br /> <span className="text-sm">Puedes asociarte a uno desde la sección Socios en la pagina principal.</span></div>
      ) : (
