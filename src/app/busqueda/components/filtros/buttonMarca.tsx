...
// Reset selectedIndex cuando cambian las marcas
  useEffect(() => {
    setSelectedIndex(-1);
  }, [marcas]);
  ...
  const handleMarcaSelect = (marca: Marca) => {
    if (!navigator.onLine) {
    alert("Error de conexión. Verifique su red."); // o reemplázalo con un `setError` si tienes estado
    return;
    }
            ...
            <Input
              ...
              onKeyDown={(e) => {
                switch(e.key) {
                  case 'ArrowDown':
                    e.preventDefault();
                    setSelectedIndex(prev => Math.min(prev + 1, marcas.length - 1));
                    break;
                  case 'ArrowUp':
                    e.preventDefault();
                  ...
                  case 'Enter':
                    e.preventDefault();
                    if (selectedIndex >= 0 && marcas[selectedIndex]) {
            ...
            <div className="py-1">
              {marcas.map((marca) => (
                <button
                  ...
                  className={`w-full px-4 py-3 text-left hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none transition-colors ${
                    selectedIndex === marcas.indexOf(marca) ? 'bg-accent text-accent-foreground' : ''
                  }`}
                >
