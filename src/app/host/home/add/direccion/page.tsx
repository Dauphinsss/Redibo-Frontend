...
import CampoMapa from "../../../components/Direccion/CampoMapa";
...
export default function AddDireccion() {
  ...
  const handleDepartamentoChange = useCallback((value: string, name?: string) => {
  ...
  const handleUbicacionSelect = useCallback((lat: number | null, lng: number | null) => {
    ...
    setMapaError(lat && lng ? "" : "Debe seleccionar una ubicación en el mapa");
  }, []);
  ...
  }, [pais, departamento, provincia, calle, numCasa, paisError, departamentoError, provinciaError, calleError, numCasaError, ubicacion]);
  ...
  onDepartamentoChange={(value, name) => {
    handleDepartamentoChange(value);
    if (name) setSelectedDepartmentName(name);
  }}
...
/>
  ...
  onValidationChange={(isValid) => {
    setMapaError(isValid ? "" : "La ubicación no coincide con el departamento seleccionado");
  }}
  initialPosition={
