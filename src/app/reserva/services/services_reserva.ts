...
import {
  ValidarInterface,
  SeguroRawRecode,
} from "@/app/reserva/interface/CoberturaForm_Interface_Recode";
...
import { ComentarioConCalificacion } from "../interface/CalifComenRecode";
...
export async function getCarRatingsFromComments(
  idCar: string
): Promise<number[]> {
  const response = await fetch(
    `https://search-car-backend.vercel.app/comments/${idCar}`
  );
  if (!response.ok) return [];

  const data: { calificacion: number }[] = await response.json();

  return data
    .filter((comentario) => comentario.calificacion > 0)
    .map((comentario) => comentario.calificacion);
}

export async function getTotalCommentsCount(idCar: string): Promise<number> {
  const response = await fetch(
    `https://search-car-backend.vercel.app/comments/${idCar}`
  );
  if (!response.ok) return 0;

  const data: { comentario: string }[] = await response.json();

  return data.filter((comentario) => comentario.comentario?.trim() !== "")
    .length;
}

export const getDetalleHost_Recode = async (id_host: number) => {
  try {
    const response = await apiCarById.get<RawHostDetails_Recode>(
      `/detailHost/${id_host}`
    );
...
export async function getCondicionesUsoVisual_Recode(
  id_carro: number
): Promise<CondicionesUsoResponse | null> {
  try {
    const response = await apiCarById.get<RawCondicionesUsoResponse>(
      `/useConditon/${id_carro}`
    );
...
export const getInsuranceByID = async (
  id_carro: string
): Promise<ValidarInterface | null> => {
  try {
    const response = await apiCobertura.get<SeguroRawRecode[]>(
      `/insurance/${id_carro}`
    );
    ...
    console.error(
      `Error inesperado al obtener el auto con ID ${id_carro}:`,
      error
    );
    throw error;
  }
...
export const getCalificacionesHost = async (id_host: number) => {
  try {
    const response = await apiRecodePuntos.get(
      `/userhost/calificacionesGet/${id_host}`
    );
...
export const getComentariosHost = async (id_host: number) => {
  try {
    const response = await apiRecodeComentario.get(
      `/userhost/comentarioGet/${id_host}`
    );
    return response.data;
  } catch (error) {
...
export const postComentarioHost = async (
  id_host: number,
  id_renter: number,
  comentario: string
) => {
  try {
    const response = await apiRecodeComentario.post(
      "/userhost/comentarioHostPost",
      {
        id_host,
        id_renter,
        comentario,
...
export const postCalificacionHost = async (
  id_host: number,
  id_renter: number,
  calificacion: number
) => {
  try {
    const response = await apiRecodePuntos.post(
      "/userhost/calificacionesPost",
      {
        id_host,
        id_renter,
        calificacion,
